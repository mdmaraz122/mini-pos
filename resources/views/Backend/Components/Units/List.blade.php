        ...
        <div class="section-body">
            <div class="row">
                <div class="col-md-12 text-right">
                    <button class="btn btn-md btn-danger" type="button" data-toggle="modal" data-target="#createUnits">Add New</button>
                </div>
            </div>
            <div class="row mt-3">
                <div class="col-12">
...
<script>
    ...
    async function getData() {
        try {
            showLoader();
            let res = await axios.get("/backendData/units-list");
            hideLoader();
            ...
            $(document).off('click', '.editBtn').on('click', '.editBtn', async function () {
                let id = $(this).data('id');
                await GetEditBaseUnitData(id);
                $('#EditUnitsModel').modal('show');
            });
            ...
            $('#table-2').DataTable({
                ...
                order: [[0, "desc"]], // Sort by 'Name' column or change to desired field
                pageLength: 10,
    ...
    async function itemDelete(id) {
        try {
            ...
            if (willDelete) {
                showLoader();
                var res = await axios.post("/backendData/units-delete/", { id: id });
                if (res.data['status'] === 'success') {
                    successToast('Unit Has Been Deleted Successfully');
                    $('#table-2').DataTable().destroy(); // Clear old datatable
                    await getData(); // Reload table
                } else {
                    hideLoader();
                    errorToast('Unable to delete this unit');
                }
            } else {
